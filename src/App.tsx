...
export default function App() {
  // Stars - generate all with unique base index
  const farStars = Array.from({ length: 120 }, (_, i) => createStar(0));
  const midStars = Array.from({ length: 80 }, (_, i) => createStar(1));
  const nearStars = Array.from({ length: 40 }, (_, i) => createStar(2));

  const [ssList, setSSList] = useState<SS[]>([]);
  ...
  const fireSS = useCallback(() => {
    ssIdRef.current += 1;
    const ss = makeSS(ssIdRef.current);
    setSSList(prev => [...prev.slice(-4), ss]);
    setTimeout(() => setSSList(prev => prev.filter(s => s.id !== ssIdRef.current)), 2000);
  }, []);

  useEffect(() => {
    const interval = setInterval(() => { if (Math.random() > 0.45) fireSS(); }, 7000);
  ...
  return (
    <div className="page">
      ...
      {farStars.map((s, i) => {
        ...
        return (
          <div key={`far-${i}`} className="star star-far"
            style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.r * 2, height: s.r * 2, background: s.color, opacity: Math.max(0.01, Math.min(1, a)) }}
            aria-hidden="true" />
        );
      })}
      ...
      {midStars.map((s, i) => {
        const a = s.alpha * (0.3 + 0.7 * Math.sin(tick * s.twinkleSpeed + s.twinklePhase));
        return (
          <div key={`mid-${i}`} className="star star-mid"
            style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.r * 2, height: s.r * 2, background: s.color, opacity: Math.max(0.02, Math.min(1, a)), boxShadow: s.glow ? `0 0 ${s.r * 3}px ${s.color}55` : 'none' }}
            aria-hidden="true" />
        );
      })}
      ...
      {nearStars.map((s, i) => {
        ...
        return (
          <div key={`near-${i}`} className="star star-near"
            style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.r * 2, height: s.r * 2, background: s.color, opacity: Math.max(0.03, Math.min(1, a)), boxShadow: s.glow ? `0 0 ${s.r * 4}px ${s.color}, 0 0 ${s.r * 8}px ${s.color}44` : `0 0 ${s.r * 2}px ${s.color}88` }}
            aria-hidden="true" />
        );
      })}
      ...
      {ssList.map(ss => {
        ...
        return (
          <div key={`ss-${ss.id}`} aria-hidden="true" style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 5 }}>
            ...
            <div style={{
              position: 'absolute',
              left: x - ss.width * 2, top: y - ss.width * 2,
              width: ss.width * 4, height: ss.width * 4,
      ...
      <div className="content">
        ...
        <motion.div className="links-section"
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.9 }}>
          <span className="links-hint">
            <span className="hint-dot" />新域名入口
          </span>
          <div className="links-row">
            <motion.a href="https://cot.wiki" target="_blank" rel="noreferrer"
            ...
            <motion.a href="https://blog.cot.wiki" target="_blank" rel="noreferrer"
      ...
      <motion.footer className="footer"
        ...
        <a href="mailto:Cotovo@163.com" className="fl"><Mail size={9} />Cotovo@163.com</a>
        <span className="fd">·</span>
        <a href="https://beian.miit.gov.cn/" target="_blank" rel="noreferrer" className="fl">鄂ICP备2025157857号</a>
      </motion.footer>
    </div>
  );
}
